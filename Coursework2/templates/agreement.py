...
def V_phrase_num(tr):
    ...
    if (tr.label() == 'T' or tr.label() == 'I'):
        ...
        if (len(tr) == 1):
            ...
            return ""# return V_phrase_num(tr[0])
        # else :
        #     return ""
    else:
        return ""
...
def restore_words(tr,wds):
    """adds wo (tr[0] == "Nom"):
            return N_phrase_num(tr[0])
            rds back into syntax tree, sometimes tagged with POS prefixes"""
    wdscopy = wds+[]
    wdscopy.reverse()
...
if __name__ == "__main__":
    ...
    #tr0 = all_valid_parses(lx, ['Who','is','a','cute','duck','?'])[0]
    #tr = restore_words(tr0,['Who','is','a','cute','duck','?'])
    #tr0 = all_valid_parses(lx,['Which','duck','flies','and','likes','a','dog','who','swims','?'])[0]
    #tr = restore_words(tr0,['Which','duck','flies','and','likes','a','dog','who','swims','?'])
    # tr0 = all_valid_parses(lx, ['Who','is','a', 'duck','?'])[0]
    # tr = restore_words(tr0,['Who','is','a', 'duck','?'])
    # tr0 = all_valid_parses(lx, ['Who','likes','Mary','?'])[0]
    # tr = restore_words(tr0,['Who','likes','Mary','?'])
    tr.draw()

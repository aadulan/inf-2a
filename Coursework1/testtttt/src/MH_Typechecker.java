...
class MH_Typechecker {
	...
	throws TypeError, UnknownVariable {
    	if( exp == null) {
    		throw new TypeError("null variable") ;
    	}else {
    	
    		if(exp.isVAR()) {
    			return env.typeOf(exp.value()); 
    		} else if (exp.isNUM()) {
    			String expression = exp.value();
    			
    			for(char c : expression.toCharArray()) {
    				if(c<'0' || c>'9') 
    				{
    					throw new TypeError ("Value of " + c + " is not numerical in " + expression);
    				}
    			}
    		 return IntegerType ;
    			...
    			if(!exp.value().equals("True") || !exp.value().equals("False")) {
    				throw new TypeError ( exp.value() + " is not a BOOLEAN value");
    			}
    			...
    			if(!first.isFun()) {
    				throw new TypeError ( exp.first().value() + " is not a function");
    			}
    			if (!(second.equals(first.left()))) {
    				throw new TypeError (exp.second().value() + " cannot be an argument") ;
    				}
    			...
    			throw new TypeError(" Cannot do infix operation on " + exp.value());
    			
    		} else if (exp.isIF()) {
    			
    			MH_TYPE first = computeType(exp.first(),env);
    			MH_TYPE second = computeType(exp.second(),env);
    			MH_TYPE third  = computeType(exp.third(),env);
    			
    			if (!first.isBool()) 
    			{
    				throw new TypeError (exp.first().value() + " not a boolean expression");
    			}
    			if(!third.equals(second)) {
    				throw new TypeError (exp.second().value() + "and" + exp.third().value() + " not the same type");
    			}
    			return third;
    			
    			
    		} throw new UnknownVariable("Not a type") ;
    	}
    }
